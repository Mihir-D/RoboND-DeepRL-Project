...
void ArmPlugin::onCollisionMsg(ConstContactsPtr &contacts)
{
	...
	for (unsigned int i = 0; i < contacts->contact_size(); ++i)
	{
		...
		*/
		...
		bool collisionCheck = (contacts->contact(i).collision2().find("gripper") != std::string::npos);
		
		if( /*(strcmp(contacts->contact(i).collision1().c_str(), COLLISION_ITEM) == 0) && 
			( strcmp(contacts->contact(i).collision2().c_str(), COLLISION_POINT) == 0)*/
			collisionCheck)
		{
			// Check the gripper collision with the object
                	rewardHistory = REWARD_WIN;// + (100.0f-episodeFrames);
                	newReward  = true;
                	endEpisode = true;
                        return;
		...
		{
			rewardHistory = REWARD_LOSS*(1+distGoal1);
			newReward = true;
			endEpisode = true;
			std::cout<<"collision element: "<<contacts->contact(i).collision2().c_str()<<"\n";
		}
...
void ArmPlugin::OnUpdate(const common::UpdateInfo& updateInfo)
{
	...
	if( hadNewState && !newReward )
	{
		...
		*/ 
		...
		//const math::Box& objectBox = object->GetBoundingBox();
		else
		{
			const float distGoal = BoxDistance(gripBBox, propBBox);
			distGoal1 = distGoal;
			//const float distMiddle = BoxDistance(middleBBox, propBBox); // compute the reward from distance to the goal
			...
			if( episodeFrames > 1)
			{
				...
				rewardHistory = avgGoalDelta; //0.125;

				//rewardHistory = distDelta*75;
				//rewardHistory = distDelta*exp(-distDelta*10);
				//rewardHistory = distDelta * 900 + 1000*(distLink2 -distGoal) + 100*(distMiddle-distGoal);
			//	std::cout<<"reward---- "<<rewardHistory<<"---";
				//if( avgGoalDelta < 0){
				//	rewardHistory *= -1.0f;//1.5*distGoal;
				//}
				newReward     = true;
				//if( avgGoalDelta < 0.001 )
				//	rewardHistory *= -1;
				//if( abs(avgGoalDelta) < 0.001f ){
                                //	rewardHistory = -0.125f;
                                //}	
			}
